...
local function fishCheckLoop()
    ...
    while running and fishCheckEnabled do
        ...
        if currentTime - lastFishTime >= 5 and lastFishTime > 0 then
...
--// SUPER BLATANT FISHING
local Section = Auto:Section({ 
    Title = "Super Blatant Fishing",
})

local superBlatantRunning = false
local superBlatantDelay = 0.1
local superBlatantEquipped = false
...
local SuperBlatantToggle = Auto:Toggle({
    Title = "Super Blatant Fishing",
    Type = "Toggle", 
    Desc = "ULTRA FAST FISHING - VERY BLATANT!",
    Default = false,
    Callback = function(state)
        superBlatantRunning = state
        if state then
            -- Matikan auto fishing biasa jika nyala
...
local function createNormalEmbed(fishName, playerName, tierName, tierColor, tierEmoji, weight, mutationStatus, fishPrice)
    return {
        ...
        footer = {
            text = "Draa Community • " .. os.date("%m/%d/%Y %I:%M %p")
        },
        timestamp = os.date("!%Y-%m-%dT%H:%M:%SZ")
    }
end
...
local function testWebhookConnection()
    ...
    local payload = {
        username = "DraaCommunity",
        embeds = {
            {
                title = "✅ Webhook Connected",
                description = "DraaCommunity webhook system is now connected and ready!",
                color = 0x00FF00,
                fields = {
                    {
                        name = "**Status**",
                ...
                footer = {
                    text = "DraaCommunity • " .. os.date("%m/%d/%Y %I:%M %p")
                },
                timestamp = os.date("!%Y-%m-%dT%H:%M:%SZ")
            }
        }
...
local DraaConfig = ConfigManager:CreateConfig("FishingConfig")

local function registerAllElements()
    
    DraaConfig:Register("Auto_FishingToggle", autoFishingToggle)
    DraaConfig:Register("LocationDropdown", LocationDropdown)
    DraaConfig:Register("TeleportToggle", TeleportToggle)
    DraaConfig:Register("AutoSellSlider", AutoSellSlider)
    DraaConfig:Register("AutoSellToggle", AutoSellToggle)
    DraaConfig:Register("AutoFavRarity", AutoFavRarity)
    DraaConfig:Register("AutoFavMutasi", AutoFavMutasi)
    DraaConfig:Register("AutoFAVORITE", AutoFAVORITE)
    DraaConfig:Register("Shop_WeatherDropdown", WeatherDropdown)
    DraaConfig:Register("Shop_AutoWeatherToggle", AutoWeatherToggle)
    DraaConfig:Register("GameToggle", GameToggle)
    DraaConfig:Register("GameEventToggle", GameEventToggle)
    DraaConfig:Register("DeepSeaToggle", DeepSeaToggle)
    DraaConfig:Register("ElementJungleToggle", ElementJungleToggle)
    DraaConfig:Register("AnimatorToggle", AnimatorToggle)
    DraaConfig:Register("WebhookInput", WebhookInput)
    DraaConfig:Register("TierDropdown", TierDropdown)
    DraaConfig:Register("MonitorToggle", MonitorToggle)
end

local function setupConfigButtons()
    local SaveConfigButton = Config:Button({
        Title = "Save Config",
        Desc = "Save ALL current settings",
        Callback = function()
            DraaConfig:Save("DraaFishConfig")
            WindUI:Notify({
                Title = "Config Saved",
    ...
    local LoadConfigButton = Config:Button({
        Title = "Load Config",
        Desc = "Load ALL saved settings",
        Callback = function()
            DraaConfig:Load("DraaFishConfig")
            WindUI:Notify({
                Title = "Config Loaded",
    ...
    local ResetConfigButton = Config:Button({
        Title = "Reset Config",
        Desc = "Reset to default settings",
        Callback = function()
            DraaConfig:Delete("DraaFishConfig")

            for _, element in pairs(DraaConfig.Elements or {}) do
...
task.spawn(function()
    ...
    if DraaConfig.ConfigExists and DraaConfig:ConfigExists("DraaFishConfig") then
        DraaConfig:Load("DraaFishConfig")
    end
end)